...
$HERMES keys add --chain stargaze-1 --key-name hrelayer --mnemonic-file $FOLDER/relayer-mnemonic || true

# Create key in hermes for osmo-testing chain with the name relayer2
$HERMES keys add --chain joe-1 --key-name hrelayer --mnemonic-file $FOLDER/relayer-mnemonic || true

# Lets start a connection between our 2 chains, make sure the chains are running or else this will not work.
$HERMES create connection --a-chain stargaze-1 --b-chain joe-1;
